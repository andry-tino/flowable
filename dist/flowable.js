(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["flowable"] = factory();
	else
		root["flowable"] = factory();
})(this, function() {
...
var arrangement = exports.arrangement = {
...
var connectivity = exports.connectivity = {
    ...
    _createClass(Connector, [{
        ...
        value: function connect(A, B) {
            ...
            if (B.y >= A.y + A.height && B.x >= A.x + A.width) {
                ...
                var element = connect_a_top_left_b_bottom_right(A, B);
                return;
            }
            if (B.y >= A.y + A.height && B.x >= A.x) {
                console.log("connector", "running B.y >= A.y + A.height && B.x >= A.x + A.widt");
                var _element = connect_a_top_left_b_bottom_right(A, B);
                return;
            }

            if (B.y >= A.y + A.height && B.x + B.width <= A.x) {
                console.log("connector", "running B.y >= A.y + A.height && B.x + B.width <= A.x");
                var _element2 = connect_a_top_right_b_bottom_left(A, B);
                return;
            }
            if (B.y >= A.y + A.height && B.x <= A.x) {
                console.log("connector", "running B.y >= A.y + A.height && B.x + B.width <= A.x");
                var _element3 = connect_a_top_right_b_bottom_left(A, B);
                return;
            }

            if (B.y + B.height <= A.y && B.x + B.width <= A.x) {
                console.log("connector", "running B.y + B.height <= A.y && B.x + B.width <= A.xx");
                var _element4 = connect_a_bottom_right_b_top_left(A, B);
                return;
            }
            if (B.y + B.height <= A.y && B.x <= A.x) {
                console.log("connector", "running B.y + B.height <= A.y && B.x + B.width <= A.xx");
                var _element5 = connect_a_bottom_right_b_top_left(A, B);
                return;
            }

            if (B.y + B.height < A.y && B.x >= A.x + A.width) {
                console.log("connector", "running B.y + B.height < A.y && B.x >= A.x + A.width");
                var _element6 = connect_a_bottom_left_b_top_right(A, B);
                return;
            }
            if (B.y + B.height < A.y && B.x >= A.x) {
                console.log("connector", "running B.y + B.height < A.y && B.x >= A.x + A.width");
                var _element7 = connect_a_bottom_left_b_top_right(A, B);
                return;
            }

            if (B.x >= A.x + A.width && B.y < A.y + A.height) {
                console.log("connector", "running B.x >= A.x + A.width && B.y < A.y + A.height");
                var _element8 = connect_a_left_b_right(A, B);
                return;
            }
            if (B.x >= A.x + A.width && B.y + B.height < A.y) {
                // Useless, covered by previous, here for safety
                console.log("connector", "running B.x >= A.x + A.width && B.y + B.height < A.y ! 2");
                var _element9 = connect_a_left_b_right(A, B);
                return;
            }

            if (B.x + B.width <= A.x && B.y < A.y + A.height) {
                console.log("connector", "running B.x + B.width <= A.x && B.y < A.y + A.height");
                var _element10 = connect_a_right_b_left(A, B);
                return;
            }
            if (B.x + B.width <= A.x && B.y + B.height < A.y) {
                // Useless, covered by previous, here for safety
                console.log("connector", "running B.x + B.width <= A.x && B.y + B.height < A.y");
                var _element11 = connect_a_right_b_left(A, B);
                return;
            }
...
function connect_a_top_left_b_bottom_right(A, B) {
    var q = quantities(A, B);

    var x = q.xa1 + Math.floor(A.width / 2);
    var y = q.ya2;
...
function connect_a_top_right_b_bottom_left(A, B) {
    ...
    var x = q.xb1 + Math.floor(B.width / 2);
    var y = q.ya2;
...
function connect_a_left_b_right(A, B) {
    ...
    var x = q.xa2;
    var y = q.ya1 + Math.floor(A.height / 2) - (A.y <= B.y ? 0 : q.h2);

    return A.y <= B.y ? generateLine(q.w2, q.h2, y, x, 0, 0, q.w2, q.h2) : generateLine(q.w2, q.h2, y, x, 0, q.h2, q.w2, 0);
}
...
function connect_a_right_b_left(A, B) {
    ...
    var x = q.xb2;
    var y = q.yb1 + Math.floor(B.height / 2) - (A.y >= B.y ? 0 : q.h3);

    return A.y >= B.y ? generateLine(q.w3, q.h3, y, x, 0, 0, q.w3, q.h3) : generateLine(q.w3, q.h3, y, x, 0, q.h3, q.w3, 0);
}
...
function generateLine(w, h, top, left, l1, l2, l3, l4) {
    var svg = (0, _snapsvg2.default)(w, h);
    var svgId = "conn" + generateId();
    svg.attr({ id: svgId });
